...
class Mic:
    ...
    def __init__(self, profile, speaker, passive_stt_engine,
        ...
        if 'sound_card' in profile:
            sound_card = {'input':"",'output':"",'input_volume':0.5, 'output_volume': 0.5 }
            ...
            sound_card['output_volume'] = profile['sound_card']['output_volume']
            self._logger.info("Setting Sound Input Device : %s, Volume: %s",sound_card['input'], sound_card['input_volume'])
            self._logger.info("Setting Sound Output Device : %s Volume: %s",sound_card['output'], sound_card['output_volume'])
            #载入pulse驱动
            ...
            for k, v in enumerate(source_list):
                if sound_card['input'] == v.name:
                   index_input = k
                   self._logger.info("Loading Sound Input List %s <Current Config Set> : name:%s , description: %s ", k, v.name,v.description)
                else:
                    self._logger.info("Loading Sound Input List %s : name: %s , description: %s ", k, v.name,v.description)

            for k, v in enumerate(sink_list):
                if sound_card['output'] == v.name:
                    index_output = k
                    self._logger.info("Loading Sound Output List %s <Current Config Set> : name:%s , description: %s ", k, v.name,v.description)
                else:
                    self._logger.info("Loading Sound Output List %s : name: %s , description: %s ", k, v.name,v.description)
            ...
            self._logger.info("User Default Sound Output Device : %s", default_sink_name)
            self._logger.info("User Default Sound Input Device : %s", default_source_name)
            #根据当前配置文件设置音频设备
            ...
            self._pulse.volume_set(source_list[index_input], input_volume) #设置输入音量
            self._pulse.volume_set(sink_list[index_output], output_volume) #设置输出音量
            ...
            input_volume = source_list[index_input].volume
            output_volume = sink_list[index_output].volume
            self._logger.info("Robot Current Sound Input Device : %s ,Volume: %s", default_source_name, input_volume)
            self._logger.info("Robot Current Sound Output Device : %s ,Volume: %s", default_sink_name, output_volume)
